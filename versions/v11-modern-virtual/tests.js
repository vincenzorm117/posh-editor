...
function createTestResultsTable() {
  ...
  container.className = 'max-h-[50vh] max-w-200 mx-auto overflow-auto bg-white shadow-2xl border-t-4 border-indigo-500 z-50';
...
function addPendingTestRow(tbody, index, testcase) {
  ...
  const statusIcon = '<span class="inline-flex items-center justify-center w-4 h-4 rounded-full bg-yellow-500 text-white text-[10px] animate-pulse">⏳</span>';
  
  const selectionRange = `[${testcase.start}, ${testcase.end}]`;
  const selectionText = testcase.selected || '(empty)';
  
  const commandsList = testcase.commands.map(cmd => `<span class="bg-purple-100 text-purple-800 px-1 py-0.5 rounded text-[10px] font-mono">${escapeHtml(cmd)}</span>`).join(' ');
...
function updateTestResult(row, index, testcase, actualHTML, actualSelection, passed) {
...
function updateSummary(passed, failed, current, total) {
  ...
  const progressText = current < total ? `Test ${current + 1} of ${total} | ` : '';
  summary.innerHTML = `
...
const waitAtBreakpoint = 'bold-only';
...
async function runTests() {
  ...
  for (let index = 0; index < tests.length; index++) {
    ...
    // Convert the before HTML to virtual DOM and update editor state
    const api = window.editorAPI;
    if (api) {
      // Temporarily set innerHTML to parse it into virtual doc
      editor.innerHTML = testcase.before;
      // Parse the DOM into virtual document format
      api.state.doc = api.parseEditorToDoc(editor);
      api.state.index = api.vBuildIndex(api.state.doc);
      api.state.vTree = null; // Reset vTree to force full re-render
      // api.state.vSel = { anchor: { pos: 0 }, focus: { pos: 0 } };
      // Render from virtual DOM to actual DOM
      api.render();
    } else {
      // Fallback: direct innerHTML set
      editor.innerHTML = testcase.before;
    }
    ...
    // Get resulting HTML
    // const afterHTML = api.cleanHtml(editor).innerHTML;
    const afterHTML = api.vRenderHtmlClean(api.state.doc);
    // Check results
