import { VirtualSelectionMarkValue } from '@/constants';
import render from '@/render/render';
import applyVirtualMarksInRange from '@/utils/applyVirtualMarksInRange';

const apply = (vState: VirtualState): any => {
  const { vSel } = vState;

  if (!vSel.isInEditor || vSel.isCollapsed) return null;

  // Determine if virtual selection already underlined
  const isAllUnderlined = vSel.marks.underline === VirtualSelectionMarkValue.ON;

  // Toggle underline mark on selected inlines
  applyVirtualMarksInRange(vState, { underline: !isAllUnderlined });

  // Render updated virtual document
  render(vState);
};

export default apply;
